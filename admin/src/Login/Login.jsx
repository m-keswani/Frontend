  ...
  const logIn = async () => {
    ...
    try {
      ...
      if (responseData.user_exists) {
        ...
        updateLog.append('uname', username)
        try{
          const response = await fetch('http://localhost:8000/updateadminlog/',{method:'PUT',body:updateLog})
          if(response.ok){
            //alert('Log Updated')
            window.location='/'       
        ...
        <div className="administration-container card custom-card shadow">
          ...
          <div className="card-body d-flex flex-column align-items justify-content-center " id="vh-200" >
            ...
            <div className="text-center mt-5">
              <button type="submit" className="button " onClick={logIn}>Log In</button>
            </div>
          </div>
        </div>
      </div>
